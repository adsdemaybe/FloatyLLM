...
__device__ __forceinline__ float block_reduce(float v) {
    ...
    for (int s = THREADS / 2; s > 0; s >>= 1) { if (t < s) sh[t] += sh[t + s]; __syncthreads(); }
    return sh[0];
}

// One block per (output row o, token tok). y[tok,o] = sum_i d_blk * q * x[tok,i].
__global__ void gemv_q8_0_kernel(const uint8_t* W, const __half* x, __half* y,
    ...
    for (int b = threadIdx.x; b < nb; b += THREADS) {
        ...
        const __half* xb = xt + b * 32;
        float s = 0.0f;
        #pragma unroll
        for (int j = 0; j < 32; ++j) s += (float)q[j] * __half2float(xb[j]);
        acc += d * s;
    }
    acc = block_reduce(acc);
    if (threadIdx.x == 0) y[(size_t)tok * n_out + o] = __float2half(acc);
}
...
// Q4_K super-block: __half d, dmin; uint8_t scales[12]; uint8_t qs[128]. 144 bytes / 256 vals.
__global__ void gemv_q4_K_kernel(const uint8_t* W, const __half* x, __half* y,
    ...
    const __half* xt = x + (size_t)tok * n_in;
    float acc = 0.0f;
    for (int sb = threadIdx.x; sb < nsb; sb += THREADS) {
