...
// fp16), keyed by global id = layer*n_experts + expert, LRU-evicted. A routing hit skips
// the DMA + dequant + transpose entirely. Per-slot events guard slot reuse across streams.
struct ExpertCache {
    int capacity = 0;
    size_t stride = 0;                 // fp16 elems per expert (gate|up|down = 3*dim*ef)
    __half* pool = nullptr;            // capacity * stride
    std::vector<int> slot_key;         // slot -> key (-1 empty)
    std::vector<uint64_t> slot_lru;    // slot -> last-use tick
    std::vector<int> key_slot;         // key -> slot (-1 not resident), size n_layers*n_experts
